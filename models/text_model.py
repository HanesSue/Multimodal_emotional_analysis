...
class BertTextCNNRegressor(nn.Module):
    def __init__(self, pretrained_model_name, embed_dim=768, num_filters=128, filter_sizes=[3,4,5]):
...
class text_model:
    ...
    """
    def __init__(self,type = "train",details = True):
        ...
        self.MAX_LEN = 64                                      #文本截断长度
        self.BATCH_SIZE = 32                                   #批处理大小
        self.EPOCHS = 50                                      #训练轮数
        self.LR = 2e-6                                        #学习率
        self.MODEL_NAME = 'bert-base-chinese'                  #使用BERT预训练模型名称
        self.SAVE_PATH = 'saved_model/best_text_model.pth'     #模型保存路径

        # 数据集路径
        self.data_path = 'dataset/meta.csv'

        # 标签配置
        self.DISCRETE_VALUES = [-1, -0.8, -0.6, -0.4, -0.2, 0, 0.2, 0.4, 0.6, 0.8, 1]  #离散化标签
        ...
        # 初始化数据集和数据加载器
        self.train_dataset = TextRegressionDataset(self.train_texts, self.train_labels, self.tokenizer, self.MAX_LEN)
        self.val_dataset = TextRegressionDataset(self.val_texts, self.val_labels, self.tokenizer, self.MAX_LEN)
        ...
        if self.type == "train":
            ...
            self.scheduler = get_linear_schedule_with_warmup(self.optimizer, num_warmup_steps=int(0.1 * self.total_steps), num_training_steps=self.total_steps)
            if details:
    ...
    def predict_train(self):
        ...
        for i in range(min(20, len(train_trues))):
            ...
            print(f"真实值: {raw_true:.1f} | 预测值: {raw_pred:.4f} | 离散化: {self.discretize(raw_pred):.1f}")