...
class SpecialSomoothL1Loss(nn.Module):
    ...
    def forward(self, prediction, target):
        ...
        loss[cond_h] = (
            error[cond_h] - 0.2
        )  # 或者你想要更大的惩罚，比如 error[cond_h] - 0.2 + (error[cond_h] - 1.0)
...
class VideoExtractor:
    ...
    def init_datasets(self):
        ...
        self.train_dataset = SIMSVideoDataset(
            self.train_raw, self.features_dict, num_frames=self.NUM_FRAMES
        )
        self.valid_dataset = SIMSVideoDataset(
            self.valid_raw, self.features_dict, num_frames=self.NUM_FRAMES
        )
        self.test_dataset = SIMSVideoDataset(
            self.test_raw, self.features_dict, num_frames=self.NUM_FRAMES
        )
        ...
        sample_weights = [
            class_to_weight[label] for label in all_labels
        ]  # 长度 = len(train_dataset)
        # 6. 用 sample_weights 创建 WeightedRandomSampler
        sampler = WeightedRandomSampler(
            weights=sample_weights,  # 每条样本的采样权重列表
            num_samples=len(
                sample_weights
            ),  # 每个 epoch 抽取样本总数，可以设置为 len(sample_weights)
    ...
    def init_model_for_training(self):
        ...
        if self.details:
            print("模型初始化完成，准备训练...")
            print(
                f"学习率调度器已设置：每{self.LR_SCHEDULER_PATIENCE}个epoch未改善则衰减至{self.LR_SCHEDULER_FACTOR}倍"
            )
    ...
    def train(self):
        ...
        for epoch in range(self.EPOCHS):
            ...
            with tqdm(
                total=len(self.train_loader), desc=f"Epoch {epoch+1}/{self.EPOCHS}"
            ) as pbar:
    ...
    def predict(self, video_path, openface_exe_path):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            success_count = video_processor.process_videos(
                input_path=video_path, output_dir=temp_dir
            )
            if success_count == 0:
            ...
            input_tensor = (
                torch.tensor(vision_features, dtype=torch.float32)
                .unsqueeze(0)
                .to(self.DEVICE)
            )  # shape: [1, T, D]
